    ...
    <body data-spy="scroll" data-target=".subnav" data-offset="50">
        ...
        <section class="container-fluid" style="margin-top: -30px;" id="dashboard">
            ...
            <div class="row-fluid">
                ...
                %>           
                <div class="dialog" id="<%=vst.getVisitid()%>_dialog"  title=" <img src='images/danpongclinic.png' width='120px;'  class='pull-right' style='margin-top: 30px; padding: 0px;' />">

                    <span style="padding-left: 2%" class="pull-left">
                    ...
                    <div class="center thumbnail">
                        ...
                        <form action="action/accountsaction.jsp" method="post">
                            <table class="table table-striped">
                                <thead>
                                    <tr>
                                        <th rowspan="2" style="text-align: left; color: #fff; font-size: 14px;">Details </th>
                                        <th rowspan="2" style="width: 30px; color: #fff; font-size: 14px;">Bill </th>

                                        <%
                                            if (!mgr.sponsorshipDetails(vst.getPatientid()).getType().equalsIgnoreCase("CASH")) {%>  
                                        <th  colspan="2" style="color: #fff; width: 100px; font-size: 14px;"> Co-Payment
                                        </th>
                                        <% }%>
                                ...
                                <tbody>
                                    <%
                                        double patientstotalbill = 0.0;
                                        List cardbills = mgr.listUnPaidPatientBillByPatientIDnBillType(vst.getPatientid(), "Card Bill");
                                        List registrationBills = mgr.listUnPaidPatientBillByPatientIDnBillType(vst.getPatientid(), "Registration Bill");
                                        List consultationbills = mgr.listUnPaidPatientBillByPatientIDnBillType(vst.getPatientid(), "Consultation Bill");
                                    %>

                                    <%
                                        if (cardbills.size() > 0) {
                                            for (int x = 0; x < cardbills.size(); x++) {
                                                PatientBills cardBill = (PatientBills) cardbills.get(x);

                                    %>


                                    <tr>
                                        <td class="card_row<%=vst.getVisitid()%>keys" style="text-transform: capitalize; text-align: left;">Registration Card Fee</td>
                                        <td class="card_row<%=vst.getVisitid()%>value" style="text-align: right;"><%=df.format(cardBill.getTotalBill())%> </td>
                                        <% patientstotalbill = patientstotalbill + cardBill.getTotalBill();%>
                                <input type="hidden" name="cardbillid" value="<%= cardBill.getId()%>"  />
                                </tr>
                                ...
                                <%
                                    if (registrationBills.size() > 0) {
                                        for (int x = 0; x < registrationBills.size(); x++) {


                                            PatientBills registrationBill = (PatientBills) registrationBills.get(x);
                                ...
                                <tr>
                                    ...
                                    <td class="registration_row<%=vst.getVisitid()%>value" style="text-align: right;"><%=df.format(registrationBill.getTotalBill())%></td>
                                    <% patientstotalbill = patientstotalbill + registrationBill.getTotalBill();%>

                                <input type="hidden" name="registrationbillid" value="<%=registrationBill.getId()%>" />
                                </tr>
                                ...
                                <%
                                    if (consultationbills.size() > 0) {
                                        for (int x = 0; x < consultationbills.size(); x++) {


                                            PatientBills consultationBill = (PatientBills) consultationbills.get(x);
                                            if (consultationBill != null) {
                                %>

                                <tr>
                                    <td class="consultation_row<%=vst.getVisitid()%>keys" style="text-transform: capitalize; text-align: left;">Consultation</td>
                                    <td class="consultation_row<%=vst.getVisitid()%>value" style="text-align: right;"><%=df.format(consultationBill.getTotalBill())%></td>
                                    <% patientstotalbill = patientstotalbill + consultationBill.getTotalBill();
                                    %>  <input type="hidden" name="consutlationbillid" value="<%=consultationBill.getId()%>" />
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: right; border-top: solid 2px black;"> <span class="amountduetext<%=vst.getVisitid()%>"> <%=df.format(patientstotalbill)%> </span> <input id="amountdueinput<%=vst.getVisitid()%>" name="amountdue" class="amountdueinput<%=vst.getVisitid()%>" style="color: blue;"  type="hidden" disabled="disabled" value="<%=df.format(patientstotalbill)%>" /></td>
                                ...
                                <tr>
                                    <td style="text-align: left;">  <b class="balancetextcolour<%=vst.getVisitid()%>">Balance Due </b> 


                                        <span style="margin: 0px; float: right;" class="paymentstatus<%=vst.getVisitid()%>" >
                                    ...
                                    <td style="text-align: right;" class="balancetext<%=vst.getVisitid()%>"> 
                                                ...
                                                <div style="font-family: 'Helvetica Neue',Helvetica,Arial,sans-serif;" class="row center">
                                                    ...
                                                    <table style="width: 100%" cellspacing="0">
                                                        ...
                                                        <tbody style="font: 12px lighter;  font-family: 'Helvetica Neue',Helvetica,Arial,sans-serif;">
                                                            ...
                                                            <tr>
                                                                <td class="registration_check<%=vst.getVisitid()%>keys" style="padding-left: 15px; line-height: 25px; text-transform: capitalize;">Registration Card Fee</td>
                                                                <td class="consultation_check<%=vst.getVisitid()%>value" style="text-align: right; font-weight: bold;margin-right: 20px; "><%=df.format(cardBill.getTotalBill())%></td>
                                                                <% patientstotalbill = patientstotalbill + cardBill.getTotalBill();%>
                                                            </tr>
                                                            ...
                                                            <tr>
                                                                <td class="registration_check<%=vst.getVisitid()%>keys" style="padding-left: 15px; line-height: 25px; text-transform: capitalize;">Registration</td>
                                                                <td class="registration_check<%=vst.getVisitid()%>value" style="text-align: right; font-weight: bold; margin-right: 20px;"><%=df.format(registrationBill.getTotalBill())%></td>
                                                                <% patientstotalbill = patientstotalbill + registrationBill.getTotalBill();%>
                                                            </tr>
                                                            ...
                                                            <tr>
                                                                <td class="consultation_check<%=vst.getVisitid()%>keys" style="padding-left: 15px; line-height: 18px; text-transform: capitalize; ">Consultation</td>
                                                                <td class="consultation_check<%=vst.getVisitid()%>value" style="text-align: right; font-weight: bold;margin-right: 20px; "><%=df.format(consultationBill.getTotalBill())%></td>
                                                                <% patientstotalbill = patientstotalbill + consultationBill.getTotalBill();%>
                                                            </tr>
                            ...
                            <div style="padding-left: 0px;" class="form-actions">
                                ...
                                <button style="width: 40%" id="button<%= vst.getVisitid() %>" type="submit" name="action"  value="Consultation Receipt" class="btn btn-danger pull-right">
                                    <i class="icon-white icon-arrow-right"> </i> Forward
                                </button>

                                <% } else {%> 
                                <button style="width: 40%" type="submit" name="action"   value="Consultation Receipt" class="btn btn-danger pull-right">
                                    <i class="icon-white icon-arrow-right"> </i> Forward
                                </button>
                                <% }%>
                                <div class="clearfix">
                                ...
                                <div>
                                    ...
                                    <button type="button" class="btn btn-inverse pull-left" style="color: white; width: 40%; margin-left: 30%" href="#" onclick='showdiscount("discount<%=vst.getVisitid()%>");return false' id="discount<%=vst.getVisitid()%>">
    ...
    $(function(){
        ...
        $(".amountpaidinput<%= vst.getVisitid()%>").live('keyup',function(){
            ...
            if(amountpaid > 0 && balance < 0){
                ...
                $('#button<%= vst.getVisitid() %>').attr("disabled","disabled");
                ...
                $('#button<%=vst.getVisitid()%>').removeAttr('disabled');
