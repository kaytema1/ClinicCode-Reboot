    ...
    <head>
        ...
        <script type="text/javascript">
            function submitform(){
              ...
              //  alert(t)
                if(t){
                    ...
                    $.post('action/treatgroupaction.jsp', { action : "groups", description : description}, function(data) {
                        alert(data);
            ...
            function updateUnit(code,description){
                   ...
                   // alert(uname);
                    $.post('action/treatgroupaction.jsp', { action : "editgroups", code : code, desc : desc }, function(data) {
                        alert(data);
                        $('#results').html(data).hide().slideDown('slow');
                    } );
            }
            ...
            function deleteUnit(id){
                ...
                if (con ==true)
                {
                    $.post('action/treatgroupaction.jsp', { action : "deletegroup", id : id}, function(data) {
                        alert(data);
                        $('#results').html(data).hide().slideDown('slow');
                    } );
                }
    ...
    <body data-spy="scroll" data-target=".subnav" data-offset="50">
        ...
        <div class="container-fluid">
            ...
            <section style="margin-top: -30px;" id="dashboard">
                ...
                <div class="row">
                    ...
                    <%
                       ...
                       // HMSHelper mgr = new HMSHelper();
                        ...
                        List list = mgr.listTherapeuticGroup();
                                ...
                                <fieldset>
                                    ...
                                    <div class="form-actions">
                                        ...
                                        <button class="btn btn-danger btn-large"  onclick='submitform();return false;'>
                                            <i class="icon-arrow-right icon-white"> </i> Add Therapeutic Group 
                                        </button>
                ...
                <div style="display: none;" id="dialog" title="List of All Categories">
                        ...
                        <tbody>
                            <%
                                for (int i = 0; i < list.size(); i++) {
                                    TherapeuticGroup group = (TherapeuticGroup) list.get(i);
                            %>
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-group btn-medium" id="<%=group.getTheraid()%>_link">
                                        <i class="icon-arrow-right icon-white"> </i> Edit 
                                    </button>
                                    <div style="display: none;" id="<%=group.getTheraid()%>_dialog" title="Editting <%=group.getDescription()%>">
                                        <form action="action/treatgroupaction.jsp" method="post" name="edit">
                                           
                                            <div class="control-group">
                                                <label class="control-label" for="input01"> Description </label>
                                                <div class="controls">
                                                    <input type="hidden" name="id" id= "id_<%=group.getTheraid()%>" value="<%=group.getTheraid()%>">
                                                   
                                                    <input type="text" name="desc" id= "desc_<%=group.getTheraid()%>" value="<%=group.getDescription()%>">
                                            ...
                                            <div class="form-actions">
                                                <button class="btn btn-danger btn-large" type="submit" name="action" value="units" onclick='updateUnit("id_<%=group.getTheraid()%>","desc_<%=group.getTheraid()%>");return false;'>
                                                    <i class="icon-arrow-right icon-white"> </i> Update Group 
                                                </button>
                                ...
                                <td>
                                    <form action="action/treatgroupaction.jsp" method="post">
                                        <input type="hidden" id="id_<%=group.getTheraid()%>" value="<%=group.getTheraid()%>"/> 

                                        <button class="btn btn-danger btn-medium" type="submit" name="action" value="units" onclick='deleteUnit("id_<%=group.getTheraid()%>");return false;'>
...
<script type="text/javascript">
    ...
    {
       /* var x=document.forms["items"]["code"].value;
        if (x==null || x=="")
        {
        ...
        var x=document.forms["items"]["description"].value;
        if (x==null || x=="")
        {
            // alert("Item must be filled out");
            return false;
...
<% for (int i = 0;
            i < list.size();
            i++) {
        TherapeuticGroup vst = (TherapeuticGroup) list.get(i);
%>
...
<script type="text/javascript">
    ...
    $("#<%= vst.getTheraid()%>_adddrug_dialog").dialog({
        autoOpen : false,
        width : 1000,
        modal :true
    ...
    $("#<%= vst.getTheraid()%>_link").click(function(){
      
        $("#<%=vst.getTheraid()%>_dialog").dialog('open');
    
    })
  
    
    $("#<%= vst.getTheraid()%>_adddrug_link").click(function(){
        alert("");
        $("#<%=vst.getTheraid()%>_adddrug_dialog").dialog('open');
