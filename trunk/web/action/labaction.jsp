...
<% try {
        ...
        if ("forward".equals(request.getParameter("action"))) {
            ...
            try {
                ...
                mgr.updateDiagnosticCounter(1, folderNumbering.getDiagnosticCounter() + 1);
                ...
                for (int i = 0; i < labtest.length; i++) {
                    ...
                    mgr.addPatientInvestigation(patientid, "CODE", Integer.parseInt(investigationcode[1]), "", Double.parseDouble(investigationcode[0]), id, dateFormat2.format(date2), "Pending", investigationnote, 0, laborder.getOrderid());
                    ...
                    if (!subInvsList.isEmpty()) {
                        ...
                        for (int p = 0; p < subInvsList.size(); p++) {
                            ...
                            mgr.addPatientInvestigation(patientid, "CODE", subInv.getDetailedInvId(), "", 0.0, id, dateFormat2.format(date2), "Pending", investigationnote, 0, orderid);
                        }
                    }
                    //}
                }
        ...
        if ("actionforward".equals(request.getParameter("action"))) {
            ...
            if (mgr.sponsorshipDetails(patient).getType().equalsIgnoreCase("CASH")) {
                if(Double.parseDouble(amountpaid)>Double.parseDouble(useme)){
                    mgr.updateLaborders(labid, Double.parseDouble(useme), Double.parseDouble(amountpaid), 0.0);            
                }else{
        ...
        if ("Outstanding Bills".equals(request.getParameter("action"))) {
            ...
            PaymentObject po = new PaymentObject();
            po.setTransactionId(labid);
            po.setTransactionType("Lab Payment Outstanding");
            po.setOutOfPocketId((11 + ""));
        ...
        if ("antibiotictreatment".equals(request.getParameter("action"))) {
            ...
            session.setAttribute("lasterror", "Successfully Saved");
        ...
        if ("Forward".equals(request.getParameter("laboratory"))) {
            ...
            try {
                ...
                session.setAttribute("lasterror", "Successfully forwarded");
                if(mgr.getStafftableByid(user.getStaffid()).getRole()==9){
                  response.sendRedirect("../listlabpatients.jsp");  
                  return;
                }
                response.sendRedirect("../labvetted.jsp");
                return;
        ...
        if ("fromclinic".equals(request.getParameter("action"))) {
            String patientid = request.getParameter("patient") == null ? "" : request.getParameter("patient");
            //String folder  = mgr.getLabPatientByID(patientid).getFolderNumber();
           ...
           // String y = yr.substring(2);
            ...
            try {
                ...
                if(amntPaid>amountdue){
                  mgr.updateLaborders(orderid, amountdue, amountpaid, 0.0);  
                }
                if(amntPaid<amountdue){
