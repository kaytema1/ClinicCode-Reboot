...
<html>
    <head>
        ...
        <%
            ...
            double totalOutOfPocketAmount = 0, totalSponsoredAmount = 0, totalOutstandingAmount = 0;
            
            // to find out outstanding bills owed by individuals, put all laborderids into a set, 
            // use that set to find out individual bills outstanding bills
            Set<String> uniqueLabOrderIds = new HashSet<String>();
            
            if(!payments.isEmpty()) {
                for(PaymentObject payment : payments) {
                    System.out.println("transactionid : " + payment.getTransactionId());
                    
                    if(payment.getOutOfPocketAmount() > 0) {
                        totalOutOfPocketAmount += payment.getOutOfPocketAmount();
                    }
                    
                    if(payment.getPrimarySponsorId() != null ) {
                        if(payment.getPrimarySponsorAmount() > 0) {
                            totalSponsoredAmount += payment.getPrimarySponsorAmount();
                        }
                    }
                    
                     if(payment.getSecSponsorId() != null ) {
                        if(payment.getSecSponsorAmount() > 0) {
                            totalSponsoredAmount += payment.getSecSponsorAmount();
                        }
                    }
                    
                    // if primarysponsoramount is null and secondarysponsoramount is null, thats an out of pocket only payment
                    // and that where we need to fetch outstanding information from
                    // put that transactionid into the set
                    
                    if(payment.getPrimarySponsorAmount() == null && payment.getSecSponsorAmount() == null) {
                        uniqueLabOrderIds.add(payment.getTransactionId());
                    }
                }
                
                // get information relating to how much is owed from a transaction
                if(!uniqueLabOrderIds.isEmpty()) {
                    Laborders labOrder = null;
                    for(String labOrderId : uniqueLabOrderIds) {
                        labOrder = mgr.getLabOrdersByOrderId(labOrderId);
                        
                        if(labOrder != null) {
                            totalOutstandingAmount += labOrder.getOutstanding();
                        }
                    }
                }
            }
            ...
            System.out.println("uniquelaborderids.size : " + uniqueLabOrderIds.size());
        %>
    </head>
                    ...
                    <div style="margin-top: 0px; "class="span12 offset3 content1 hide">
                        ...
                        <div style="padding-bottom: 80px;" class="span9 thumbnail well content">
                            ...
                            <table class="example display table">
                                ...
                                <tbody>
                                    <tr>
                                        <td style="line-height: 25px; text-transform: uppercase; color: #4183C4;  font-weight: bolder; font-size: 18px;">Cash At Hand</td>
                                        <td style="text-align: right;"><%=df.format(totalOutOfPocketAmount)%></td>
                                        <td style="text-align: center;"> 
                                            <a class="btn btn-info btn-small" href="cashlabsalesbystaffdetails.jsp?staffid=<%=staffid %>&datevalue=<%=displayedDate  %>">View Details </a> 
                                        </td>
                                    </tr> 
                                    <tr>
                                         <td style="line-height: 25px; text-transform: uppercase; color: #4183C4;  font-weight: bolder; font-size: 18px;">Credit ( Companies )</td>
                                        <td style="text-align: right;"><%=df.format(totalSponsoredAmount)%></td>
                                        <td style="text-align: center;"> 
                                            <!--a class="btn btn-info btn-small" href="creditlabsalesbystaffdetails.jsp?staffid=<%=staffid %>&datevalue=<%=displayedDate  %>">View Details </a-->
                                             <a class="btn btn-info btn-small" href="salesinsurancedetails.jsp?staffid=<%=staffid %>&datevalue=<%=displayedDate  %>">View Details </a>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td style="line-height: 25px; text-transform: uppercase; color: #4183C4;  font-weight: bolder; font-size: 18px;">Credit ( Individuals ) </td>
                                        <td style="text-align: right;"><%=df.format(totalOutstandingAmount)%></td>
                                        <td style="text-align: center;"> 
                                        </td>
                                    </tr>
                                </tbody>
                                <tfoot>
                                    <tr>
                                        <td></td>
                                        <td  style="text-align: right;"><b><%=df.format(totalOutstandingAmount + totalSponsoredAmount + totalOutOfPocketAmount)%></b></td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
