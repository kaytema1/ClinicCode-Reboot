    ...
    <head>
        ...
        <script type="text/javascript">
            function submitform(){
              ...
              //  alert(t)
                if(t){
                    ...
                    $.post('action/diagnisisgroupaction.jsp', { action : "groups", name : name, description : description}, function(data) {
                        alert(data);
            ...
            function updateUnit(code,description){
                   ...
                   // alert(uname);
                    $.post('action/diagnisisgroupaction.jsp', { action : "editgroups", code : code, desc : desc }, function(data) {
                        alert(data);
                        $('#results').html(data).hide().slideDown('slow');
                    } );
            }
            ...
            function deleteUnit(id){
                ...
                if (con ==true)
                {
                    $.post('action/diagnisisgroupaction.jsp', { action : "deletegroup", id : id}, function(data) {
                        alert(data);
                        $('#results').html(data).hide().slideDown('slow');
                    } );
                }
                                ...
                                <fieldset>
                                    ...
                                    <div class="form-actions">
                                        ...
                                        <button class="btn btn-danger btn-large"  onclick='submitform();return false;'>
                                            <i class="icon-arrow-right icon-white"> </i> Add Diagnostic Group 
                                        </button>
                        ...
                        <tbody>
                            <%
                                for (int i = 0; i < list.size(); i++) {
                                    DiagnosticGroupings dg = (DiagnosticGroupings) list.get(i);
                            %>
                            <tr>
                                <td>
                                    <%=dg.getCode()%>
                                ...
                                <td>
                                    <button class="btn btn-group btn-medium" id="<%=dg.getCode()%>_link">
                                        <i class="icon-arrow-right icon-white"> </i> Edit 
                                    </button>
                                    <div style="display: none;" id="<%=dg.getCode()%>_dialog" title="Editting <%=dg.getDescriptio()%>">
                                        <form action="action/unitaction.jsp" method="post" name="edit">
                                            <div class="control-group">
                                                <label class="control-label" for="input01"> Code: </label>
                                                <div class="controls">
                                                    <input type="text" name="code" id="nameunit_<%=dg.getCode()%>" value="<%=dg.getCode()%>">
                                            ...
                                            <div class="control-group">
                                                <label class="control-label" for="input01"> Description </label>
                                                <div class="controls">
                                                    <input type="text" name="ubeds" id="ubeds_<%=dg.getCode()%>" value="<%=dg.getDescriptio()%>">
                                            ...
                                            <div class="form-actions">
                                                <button class="btn btn-danger btn-large" type="submit" name="action" value="units" onclick='updateUnit("nameunit_<%=dg.getCode()%>","ubeds_<%=dg.getCode()%>");return false;'>
                                                    <i class="icon-arrow-right icon-white"> </i> Update Category 
                                                </button>
                                ...
                                <td>
                                    <form action="action/unitaction.jsp" method="post">
                                        <input type="hidden" id="id_<%=dg.getCode()%>" value="<%=dg.getCode()%>"/> 

                                        <button class="btn btn-danger btn-medium" type="submit" name="action" value="units" onclick='deleteUnit("id_<%=dg.getCode()%>");return false;'>
...
<% for (int i = 0;
            i < list.size();
            i++) {
        DiagnosticGroupings vst = (DiagnosticGroupings) list.get(i);
%>
