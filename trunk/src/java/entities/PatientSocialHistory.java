package entities;
// Generated Nov 29, 2013 11:26:33 AM by Hibernate Tools 3.2.1.GA


import java.util.Date;
import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import static javax.persistence.GenerationType.IDENTITY;
import javax.persistence.Id;
import javax.persistence.Table;
import javax.persistence.Temporal;
import javax.persistence.TemporalType;

/**
 * PatientSocialHistory generated by hbm2java
 */
@Entity
@Table(name="patient_social_history"
    ,catalog="extended"
)
public class PatientSocialHistory  implements java.io.Serializable {


     private Integer id;
     private int visitid;
     private String patientid;
     private int socialHistoryId;
     private int durationId;
     private Date date;

    public PatientSocialHistory() {
    }

    public PatientSocialHistory(int visitid, String patientid, int socialHistoryId, int durationId, Date date) {
       this.visitid = visitid;
       this.patientid = patientid;
       this.socialHistoryId = socialHistoryId;
       this.durationId = durationId;
       this.date = date;
    }
   
     @Id @GeneratedValue(strategy=IDENTITY)
    
    @Column(name="id", unique=true, nullable=false)
    public Integer getId() {
        return this.id;
    }
    
    public void setId(Integer id) {
        this.id = id;
    }
    
    @Column(name="visitid", nullable=false)
    public int getVisitid() {
        return this.visitid;
    }
    
    public void setVisitid(int visitid) {
        this.visitid = visitid;
    }
    
    @Column(name="patientid", nullable=false, length=11)
    public String getPatientid() {
        return this.patientid;
    }
    
    public void setPatientid(String patientid) {
        this.patientid = patientid;
    }
    
    @Column(name="social_history_id", nullable=false)
    public int getSocialHistoryId() {
        return this.socialHistoryId;
    }
    
    public void setSocialHistoryId(int socialHistoryId) {
        this.socialHistoryId = socialHistoryId;
    }
    
    @Column(name="duration_id", nullable=false)
    public int getDurationId() {
        return this.durationId;
    }
    
    public void setDurationId(int durationId) {
        this.durationId = durationId;
    }
    @Temporal(TemporalType.TIMESTAMP)
    @Column(name="date", nullable=false, length=19)
    public Date getDate() {
        return this.date;
    }
    
    public void setDate(Date date) {
        this.date = date;
    }




}


